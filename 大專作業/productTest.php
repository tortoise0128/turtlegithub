...
$sql = "SELECT `style` FROM `product` WHERE 1 ";
$cate = $pdo->query($sql)->fetchAll();
        ...
        <div class="col main">
            ...
            <div class="barLeft col-lg-3">
                <div class="chose">
                    <h3>Category</h3>
                    <div class="category">
                        <a type="button" class="btn-<?= $cate_id==0 ? '' : 'outline-' ?>" role="button" href="productTEST.php">
...
<script>
    ...
    const productTpl = function(obj){
        ...
        for(f=1; f<=6 ; f++){
            ...
            return `<div class="productItem style${f} color2 " data-sid="4">
                <div class="f-box">
                    <a href="product_in.php?sid=${obj.sid}">
                        <img src="img/small/${obj.img_small}.jpg">
                    </a>
                </div>
                <h4>${obj.name}</h4>
                <h5>NT.${obj.price}</h5>
    ...
    function pageBtnTpl(obj){
            ...
            return `<li class="page-item ${obj.isActive ? 'active' : '' }">
                    <a class="page-link" href="productTest.php?#${obj.i}">${obj.i}</a>
                </li>`;
        }


    function productHash(colorName){
        let h = location.hash.slice(1);
        h = parseInt(h) || 1;
        // info.innerHTML = h;

        $.get('product_api.php', {page: h, color: colorName}, function(data){
    ...
    $(".colorChoose").on('click', function() {
        var colorName = $(this).attr('class').split(" ")[0];
        productHash(colorName);
    });
